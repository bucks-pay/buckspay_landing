...
const HeroSection: React.FC = () => {
    ...
    const getCoinMarketCap = async () => {
        try {
            const response = await fetch('http://localhost:3000/coinmarketcap');
    ...
    const filteredData = cryptoData.filter(crypto => filteredSymbols.includes(crypto.symbol));
                                    ...
                                    <tr key={crypto.id} >
                                        <td>
                                            <div className={styles.cryptoInfo}>
                                                <Image src={`/icons/crypto/${crypto.symbol}.png`} alt={crypto.symbol} width={20} height={20} />
                                                <span>{crypto.symbol} {crypto.name}</span>
                                            </div>
                                        </td>
                                        <td>${crypto.quote.USD.price.toFixed(2)}</td>
                                        <td style={{ color: crypto.quote.USD.volume_change_24h < 0 ? 'red' : 'green' }}>
